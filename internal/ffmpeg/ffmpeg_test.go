...
func TestParseArgsDevice(t *testing.T) {
	...
	// [DEVICE] video will be transcoded to H264 with framerate 20, audio will be skipped
	args = parseArgs("device?video=0&framerate=20#video=h264")
	require.Equal(t, `ffmpeg -hide_banner -f dshow -framerate 20 -i video="0" -c:v libx264 -g 50 -profile:v high -level:v 4.1 -preset:v superfast -tune:v zerolatency -pix_fmt:v yuvj420p -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())

	// [DEVICE] video will be transcoded to H265 with framerate 20, audio will be skipped
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1280x720&framerate=20#video=h265#audio=pcma")
	require.Equal(t, `ffmpeg -hide_banner -f dshow -input_format mjpeg -video_size 1280x720 -framerate 20 -i video="0" -c:v libx265 -g 50 -profile:v main -level:v 5.1 -preset:v superfast -tune:v zerolatency -c:a pcm_alaw -ar:a 8000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}
...
func TestParseArgsIpCam(t *testing.T) {
	...
	// [HTTP-MJPEG] video will copy, need of set frame rate explicitly
	...
	// [HTTP-MJPEG] video will be transcoded to H264, need of set frame rate explicitly
...
func TestParseArgsAudio(t *testing.T) {
	...
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a aac -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to AAC/16000, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=aac/16000")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a aac -ar:a 16000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to OPUS, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=opus")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a libopus -ar:a 48000 -ac:a 2 -application:a voip -compression_level:a 0 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMU, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcmu")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_mulaw -ar:a 8000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMU/16000, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcmu/16000")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_mulaw -ar:a 16000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMU/48000, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcmu/48000")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_mulaw -ar:a 48000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMA, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcma")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_alaw -ar:a 8000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMA/16000, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcma/16000")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_alaw -ar:a 16000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [AUDIO] audio will be transcoded to PCMA/48000, video will be skipped
	args = parseArgs("rtsp:///example.com#audio=pcma/48000")
	require.Equal(t, "ffmpeg -hide_banner -allowed_media_types audio -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp:///example.com -vn -c:a pcm_alaw -ar:a 48000 -ac:a 1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())
}
...
func TestParseArgsHwVaapi(t *testing.T) {
	...
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1920x1080#video=h265#hardware=vaapi")
	require.Equal(t, `ffmpeg -hide_banner -hwaccel vaapi -hwaccel_output_format vaapi -f dshow -input_format mjpeg -video_size 1920x1080 -i video="0" -c:v hevc_vaapi -g 50 -bf 0 -profile:v high -level:v 5.1 -sei:v 0 -an -vf format=vaapi|nv12,hwupload -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}
...
func TestParseArgsHwV4l2m2m(t *testing.T) {
	...
	// [HTTP-MJPEG] video will be transcoded to H264, need of set frame rate explicitly
	args = parseArgs("http://example.com#video=h264#framerate=10#hardware=v4l2m2m")
	require.Equal(t, "ffmpeg -hide_banner -framerate 10 -fflags nobuffer -flags low_delay -i http://example.com -c:v h264_v4l2m2m -g 50 -bf 0 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())
	...
	// [DEVICE] MJPEG video with size 1920x1080 will be transcoded to H265
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1920x1080#video=h265#hardware=v4l2m2m")
	require.Equal(t, `ffmpeg -hide_banner -f dshow -input_format mjpeg -video_size 1920x1080 -i video="0" -c:v hevc_v4l2m2m -g 50 -bf 0 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}

func TestParseArgsHwCuda(t *testing.T) {
	// [HTTP-MJPEG] video will be transcoded to H264
	args := parseArgs("http:///example.com#video=h264#hardware=cuda")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -fflags nobuffer -flags low_delay -i http:///example.com -c:v h264_nvenc -g 50 -profile:v high -level:v auto -preset:v p2 -tune:v ll -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [HTTP-MJPEG] video will be transcoded to H264, need of set frame rate explicitly
	args = parseArgs("http://example.com#video=h264#framerate=10#hardware=cuda")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -framerate 10 -fflags nobuffer -flags low_delay -i http://example.com -c:v h264_nvenc -g 50 -profile:v high -level:v auto -preset:v p2 -tune:v ll -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [RTSP] video with rotation, should be transcoded, so select H264
	args = parseArgs("rtsp://example.com#video=h264#rotate=180#hardware=cuda")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -allowed_media_types video -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp://example.com -c:v h264_nvenc -g 50 -profile:v high -level:v auto -preset:v p2 -tune:v ll -an -vf transpose=1,transpose=1 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [RTSP] video with resize to 1280x720, should be transcoded, so select H265
	args = parseArgs("rtsp://example.com#video=h265#width=1280#height=720#hardware=cuda")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -allowed_media_types video -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp://example.com -c:v hevc_nvenc -g 50 -profile:v high -level:v auto -an -vf scale_cuda=1280:720 -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())

	// [DEVICE] MJPEG video with size 1920x1080 will be transcoded to H265
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1920x1080#video=h265#hardware=cuda")
	require.Equal(t, `ffmpeg -hide_banner -hwaccel cuda -hwaccel_output_format cuda -extra_hw_frames 2 -f dshow -input_format mjpeg -video_size 1920x1080 -i video="0" -c:v hevc_nvenc -g 50 -profile:v high -level:v auto -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}
...
func TestParseArgsHwDxva2(t *testing.T) {
	...
	// [HTTP-MJPEG] video will be transcoded to H264, need of set frame rate explicitly
	args = parseArgs("http://example.com#video=h264#framerate=10#hardware=dxva2")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel dxva2 -hwaccel_output_format dxva2_vld -framerate 10 -fflags nobuffer -flags low_delay -i http://example.com -c:v h264_qsv -g 50 -bf 0 -profile:v high -level:v 4.1 -async_depth:v 1 -an -vf hwmap=derive_device=qsv,format=qsv -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())
	...
	// [DEVICE] MJPEG video with size 1920x1080 will be transcoded to H265
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1920x1080#video=h265#hardware=dxva2")
	require.Equal(t, `ffmpeg -hide_banner -hwaccel dxva2 -hwaccel_output_format dxva2_vld -f dshow -input_format mjpeg -video_size 1920x1080 -i video="0" -c:v hevc_qsv -g 50 -bf 0 -profile:v high -level:v 5.1 -async_depth:v 1 -an -vf hwmap=derive_device=qsv,format=qsv -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}
...
func TestParseArgsHwVideotoolbox(t *testing.T) {
	...
	// [HTTP-MJPEG] video will be transcoded to H264, need of set frame rate explicitly
	args = parseArgs("http://example.com#video=h264#framerate=10#hardware=videotoolbox")
	require.Equal(t, "ffmpeg -hide_banner -hwaccel videotoolbox -hwaccel_output_format videotoolbox_vld -framerate 10 -fflags nobuffer -flags low_delay -i http://example.com -c:v h264_videotoolbox -g 50 -bf 0 -profile:v high -level:v 4.1 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}", args.String())
	...
	// [DEVICE] MJPEG video with size 1920x1080 will be transcoded to H265
	args = parseArgs("device?video=0&input_format=mjpeg&video_size=1920x1080#video=h265#hardware=videotoolbox")
	require.Equal(t, `ffmpeg -hide_banner -hwaccel videotoolbox -hwaccel_output_format videotoolbox_vld -f dshow -input_format mjpeg -video_size 1920x1080 -i video="0" -c:v hevc_videotoolbox -g 50 -bf 0 -profile:v high -level:v 5.1 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`, args.String())
}
