...
func (p *Producer) start() {
	...
	log.Debug().Msgf("[streams] start producer url=%s", Redact(p.url))
...
func (p *Producer) worker(conn core.Producer, workerID int) {
	if err := conn.Start(); err != nil {
		...
		log.Warn().Err(err).Str("url", Redact(p.url)).Caller().Send()
	}
...
func (p *Producer) reconnect(workerID, retry int) {
	...
	if p.workerID != workerID {
		log.Trace().Msgf("[streams] stop reconnect url=%s", Redact(p.url))
		return
	}

	log.Debug().Msgf("[streams] retry=%d to url=%s", retry, Redact(p.url))
...
func (p *Producer) stop() {
	...
	log.Debug().Msgf("[streams] stop producer url=%s", Redact(p.url))
