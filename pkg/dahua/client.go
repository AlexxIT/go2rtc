...
// dial establishes connection and discovers audio capabilities
func (c *Client) dial() error {
	log := app.GetLogger("dahua")
	log.Debug().Msg("[dahua] starting dial process")

	// Get audio input channel count
	inputCount, err := c.getAudioInputCount()
	if err != nil {
		log.Error().Err(err).Msg("[dahua] failed to get audio input count")
		return err
	}

	log.Debug().Int("count", inputCount).Msg("[dahua] audio input channels discovered")

	// Get audio output channel count
	outputCount, err := c.getAudioOutputCount()
	if err != nil {
		log.Error().Err(err).Msg("[dahua] failed to get audio output count")
		return err
	}

	log.Debug().Int("count", outputCount).Msg("[dahua] audio output channels discovered")

	if inputCount == 0 && outputCount == 0 {
		return errors.New("no audio channels available")
	}

	// Create media descriptor for audio output (send-only from go2rtc perspective)
	// We support both G.711 A-law and μ-law
	media := &core.Media{
		Kind:      core.KindAudio,
	...
	log.Debug().
		...
		Msg("[dahua] media configuration created")

	return nil
}

// getAudioInputCount gets the number of audio input channels
func (c *Client) getAudioInputCount() (int, error) {
	log := app.GetLogger("dahua")
	log.Debug().Msg("[dahua] getting audio input channel count")

	link := fmt.Sprintf("%s/cgi-bin/devAudioInput.cgi?action=getCollect", c.url)
	...
	log.Debug().Str("url", link).Msg("[dahua] sending audio input count request")
	...
	if res.StatusCode != http.StatusOK {
		log.Error().
			...
			Msg("[dahua] audio input count request failed")
	...
	log.Debug().Str("response", response).Msg("[dahua] audio input count response")

	// Parse result=N format
	if strings.HasPrefix(response, "result=") {
		var count int
		if _, err := fmt.Sscanf(response, "result=%d", &count); err != nil {
			return 0, err
		}
		return count, nil
	}
...
// getAudioOutputCount gets the number of audio output channels
func (c *Client) getAudioOutputCount() (int, error) {
	log := app.GetLogger("dahua")
	log.Debug().Msg("[dahua] getting audio output channel count")

	link := fmt.Sprintf("%s/cgi-bin/devAudioOutput.cgi?action=getCollect", c.url)
	req, err := http.NewRequest("GET", link, nil)
	if err != nil {
		return 0, err
	}
	...
	log.Debug().Str("url", link).Msg("[dahua] sending audio output count request")
	...
	if res.StatusCode != http.StatusOK {
		log.Error().
			Int("status", res.StatusCode).
			Str("status_text", res.Status).
			Msg("[dahua] audio output count request failed")
		return 0, errors.New(res.Status)
	}
	...
	response := string(body)
	log.Debug().Str("response", response).Msg("[dahua] audio output count response")
